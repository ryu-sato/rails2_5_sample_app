...
p ticket
  ...
  p diffset2.errors
      ...
      GeneratingTicket.with_advisory_lock("#{ticket.code}_#{ticket.host}_#{ticket.normal_log.id}_#{ticket.abnormal_log.id}") do
        GeneratingTicket.find_by!(ticket_code: ticket.code, hostname: ticket.host.to_s, normal_raw_log_id: ticket.normal_log.id, abnormal_raw_log_id: ticket.abnormal_log.id).destroy!
      end
    end
  end
end
